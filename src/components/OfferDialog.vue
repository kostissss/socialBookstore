            ...
            <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
              <DialogPanel class="relative transform overflow-hidden border-amber-800  border-4  shadow-amber-800 rounded-2xl my-2  bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                <div class="sm:flex ">
                  ...
                  <div class="mt-3 mb-3 flex-auto text-center sm:ml-4 sm:mt-0 sm:flex sm:flex-col">
                    
                    <DialogTitle as="h3" class="text-base text-center font-semibold leading-6 text-gray-900">{{offer.title}}</DialogTitle>
                    ...
                    <p class="text-center flex-auto ">Author: {{ offer.author }}</p>
                    <p class="text-center">Category: {{ offer.category }}</p>
                    <p class="text-center">Created At: {{ offer.createdAt + offer.id}}</p>
                ...
                <div class="mt-5 sm:mt-4 sm:flex  ">
                  ...
                  <button type="button" class="inline-flex w-full  rounded-md bg-amber-800 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-amber-500 sm:ml-auto sm:w-auto" @click="requestBook()">Request Book</button>
                </div>
              </DialogPanel>
            </TransitionChild>
          </div>
